...
## Trì hoãn thực hiện khởi tạo trong thực hành
...
-->
Về mặt lý thuyết, hiện giờ chúng ta biết nó làm việc như thế nào, hãy xem việc khởi tạo khi thực sự được kích hoạt.
Để làm như vậy, chúng ta cần lên khung một biến khởi tạo không làm gì ngoài việc gửi thông điệp gỡ lỗi cho biết khi nào đó được gọi và sử dụng các tham số nào.
...
Lưu ý rằng, mặc dù `MyInit` sẽ in thông tin về các tham số mô hình khi nó được gọi, hàm khởi tạo ở trên không in bất cứ thông tin nào cho tới khi nó thực thi. 
Do đó không có bất cứ tham số thật nào khi thực hiện gọi hàm `initialize`. 
...
Khi thực hiện tính toán tiếp theo dựa trên biến đầu vào `x`, hệ thống có thể tự động tham chiếu kích thước các tham số của tất cả các lớp dựa trên kích thước của biến đầu vào này. 
Ngay khi hệ thống đã tạo ra các tham số trên, nó gọi `MyInit` để khởi tạo chúng trước khi xử lý tính toán kế tiếp.
*dịch đoạn phía trên*
